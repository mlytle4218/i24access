...
class NewsStreamer:
    ...
    def auth(self):
        ...
        url = ('https://cleeng.com/auth/2/purchase/?v=3.0&appId=35e97a6231236gb456heg6bd7a6bdsf7&offerId=S920352949'
            ...
            random.random())
        #*****
        # looks like random not needed here anymore - not doing anythin
    ...
    def get_brightcove_new(self,assetId):
        url = 'https://players.brightcove.net/5377161796001/default_default/index.min.js'
        headers = {
            'user-agent': USER_AGENT,
            'dnt': '1',
        ...
        r = self.network.get(url, headers, render=False)
        content = MyNetwork.get_text_content(r)
        policy_key =  re.search('policyKey: *"([^"]*)"', content).group(1)
    ...
    def show_menu(self):
            ...
            while True:
                ...
                try:
                    ...
                    if (selection in range(len(sources))):
                        ...
                        print(url)
                        if record_option:
                            file_location = input('input file name ')
                            file_location = file_location + ".mp4"
                            print('recording ' + file_location)
                            subprocess.run(['ffmpeg','-y','-loglevel','error','-i',url,file_location])
                        # else:
                        #     subprocess.run([MPV, '--no-video', url])
                        elif record_live:
                            file_location = input('input file name')
                            file_location = file_location + '.aac'
                            ...
                            if at_job:
                                ...
                                result = "{0:0>2}:{1:0>2}:00".format(hours,minutes)
                                sched_cmd = ['at', at_command]
                                p = subprocess.Popen(sched_cmd, stdin=subprocess.PIPE, encoding='utf-8')
                                ffmpeg_command = 'ffmpeg' + ' -y' + ' -loglevel' + ' error' + ' -t ' + result + ' -i ' + url + ' ' + file_location
                                #ffmpeg_command = 'ffmpeg'',''-y'',''-loglevel'',''error'',''-t',result,'-i',url,file_location]
                                print(ffmpeg_command)
                                p.communicate(ffmpeg_command)
                                print('at job created')
                                sys.exit(0)


                            else:
                                subprocess.run(['ffmpeg','-y','-loglevel','error','-i',url,"-vn","-c:a","copy",file_location])
                                sys.exit(0)
                        else:
                            subprocess.run([MPV, '--no-video', url])
                        # try:
                        ...
                        # except NameError:
                        #     subprocess.run([MPV, '--no-video', url])
                        #     #subprocess.run(['youtube-dl','-q', url])
                        # else:
                        #      subprocess.run(['ffmpeg','-y','-i',url,record_option])
                        #     #subprocess.run(['youtube-dl','--hls-use-mpegts', url])
                        #     #subprocess.run([MPV, '--no-video', url])
                        break
                except ValueError:
                    if selection == 'x':
        ...
        def show(url):
            ...
            #*****
            # dont think show_container is used at all
...
@click.option('--credentials', default="/home/chime/virtual-python/credentials.json", show_default=True, type=click.Path(exists=True))
@click.option('--record', '-r', is_flag=True, help='Record the stream.')
@click.option('--live','-l', is_flag=True, help='Record the live stream only')
def main(verbose, credentials, record, live):
    global log_level
    global record_option
    global record_live
    global at_job
    record_option = False
    record_live = False
